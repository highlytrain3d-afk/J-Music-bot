...
Quick start
  ...
  2) pip install -U discord.py yt-dlp
  3) export DISCORD_TOKEN=YOUR_TOKEN_HERE  (or set in .env)
  4) python bot.py

Prefix & examples (prefix is `J/`):
  J/join
  J/play never gonna give you up
  J/play https://music.youtube.com/watch?v=...
  J/playlist https://www.youtube.com/playlist?list=...
  J/queue
  J/skip
  J/pause | J/resume | J/stop | J/leave | J/np

Notes on YouTube/YouTube Music
- This bot uses yt-dlp to extract streamable audio. Please respect YouTube’s Terms
  and only play content you have rights to stream in your context.
- For age‑restricted content, you may need to supply cookies to yt-dlp; this code
  supports that via an optional YTDLP_COOKIES file path (see YTDL_OPTS).
"""
...
YTDL_OPTS: Dict[str, Any] = {
    ...
    # Uncomment to use cookies if needed (set env var YTDLP_COOKIES to a file path)
...
class Track:
    ...
    async def create(cls, query: str, requester_id: int) -> "Track":
        """Resolve a query or URL into a Track using yt-dlp."""
        import yt_dlp  # local import so the module is optional until runtime

        # If it looks like a URL, yt-dlp will handle it; otherwise we use ytsearch
        ydl_opts = YTDL_OPTS.copy()
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            info = ydl.extract_info(query, download=False)

        # If it's a search, pick the first entry
        if "entries" in info:
        ...
        # For streaming, prefer 'url' from formats or top-level (yt-dlp sets this)
        stream_url = info.get("url")
        if not stream_url:
            # Fallback to bestaudio format url
...
class MusicPlayer:
    """Per-guild music player managing a queue and voice client."""
    ...
    async def _player_loop(self):
        while not self._stopped.is_set():
            try:
                ...
                # Idle timeout: leave if connected
            ...
            # Wait until playback finishes or the player is stopped
...
async def playlist_cmd(ctx: commands.Context, playlist_url: str):
    ...
    try:
        import yt_dlp
        ydl_opts = YTDL_OPTS.copy()
        ydl_opts["noplaylist"] = False  # allow playlist extraction here
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
    ...
    for ent in entries:
        try:
            ...
            # If yt-dlp didn't give a ready stream url, re-resolve quickly
            if "url" in ent and ent.get("vcodec") == "none":
                stream_url = ent["url"]
            else:
                # Resolve each entry minimally to get a stream URL
...
async def queue_cmd(ctx: commands.Context):
    ...
    items: List[Track] = []
    try:
        # Peek without removing: consume and re-add
